/**
 * Submits the `staff_advance_request_submitted_v1` WhatsApp template to Meta
 * (via Exotel) and records it in whatsapp_templates.
 *
 ...
 *
 * This template was seeded into the DB back when advance requests were built
 * (as 'pending') but was never actually submitted to Meta — it doesn't exist
 * on the WABA at all until this script's --submit is run.
 *
 *   node scripts/create-advance-request-template.mjs           # status only
 *   node scripts/create-advance-request-template.mjs --submit  # create at Meta
 */
import fs from "node:fs";
import path from "node:path";

const SUBMIT = process.argv.includes("--submit");
const TEMPLATE_NAME = "staff_advance_request_submitted_v1";
const BODY = 'Hi {{1}}, {{2}} has requested an advance of {{3}} for "{{4}}". Please review in the portal.';
const SAMPLE = ["Manoj Mishra", "Prosync AI Solutions", "Rs 45,000", "Raw material procurement"];
